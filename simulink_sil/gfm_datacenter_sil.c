...
void app_task_init(double vll_rated, double s_rated, double f_nom, double i_rated, double z_base)
{
	...
	set_modesAndProtection_CONFIG_dcOverVoltageThreshold(40e3f);
	set_modesAndProtection_CONFIG_dcUnderVoltageThreshold(23e3f);
	...
	set_dcVoltageCntl_CONFIG_kp(5e3);			// in SI
	set_dcVoltageCntl_CONFIG_voltageDeadband(1.5e3f);
	set_dcVoltageCntl_CONFIG_VllRated(VllRated);
	set_dcVoltageCntl_CONFIG_pLimitExternalMinimum(0.05f * SRated);	// prevent interference with dc-control at low power
	dcVoltageCntl_init();
...
void app_task_5kHz(double i_ref[3], bool* pwm_enable, // outputs
     	           const double v_ac[3], double P_request, double Q_request, bool enable, double P_lim_high, double P_lim_low, double v_dc, double v_dc_ref) // inputs
{
	// init
	uint16_t i;
	...
	currentSource_IN.P_request = P_request;
